 ...
 private:
  ...
  class element_hasher_adapter {
   public:
     ...
     * parent column. ii. Otherwise, push the next child column into the stack. c. If the column is
     * a primitive column, compute the hash value, pop the element, and update `cur_hash` of its
     * parent column.
